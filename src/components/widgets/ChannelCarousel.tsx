...
export function ChannelCarousel({ channels, className }: ChannelCarouselProps) {
  ...
  // Check scroll on mount and when channels change
  useEffect(() => {
    ...
    <div className={cn("relative", className)}>
      ...
      >
        {channels.map((channel, index) => (
          <YoutubeChannelCard key={index} url={channel.url} title={channel.title} />
        ))}
      </div>
    </div>
  );
