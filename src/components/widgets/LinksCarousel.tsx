...
export function LinksCarousel({ links, className }: LinksCarouselProps) {
  ...
  // Check scroll on mount and when links change
  useEffect(() => {
    ...
    <div className={cn("relative", className)}>
      ...
      >
        {links.map((link, index) => (
          <ExternalLinkCard key={index} url={link.url} title={link.title} />
        ))}
      </div>
    </div>
  );
